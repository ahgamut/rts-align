...
def show_points(Q, K, Q_corr, K_corr, clq, adjmat, filename=None):
    ...
    q_pts = ax1.scatter(
        Q_corr[:, 0], Q_corr[:, 1], s=80, c=colors[: len(Q), :], marker="o", alpha=0.5
    )
    ax1.set_title("$P$")
    k_pts = ax2.scatter(
        K_corr[:, 0], K_corr[:, 1], s=80, c=colors[len(Q) :, :], marker="o", alpha=0.5
    )
    ax2.set_title("$Q$")
...
def main():
    ...
    attempt(num_K=5, num_extra=0, noise_range=0.01, output_file=d.output_file)
