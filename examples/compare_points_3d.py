...
def make_estims(d):
    ...
    estim_objs = [
        GoICP(outlier_frac=outlier_frac),
        GoICPScaled(outlier_frac=outlier_frac),
        TEASER(noise_range=d.noise_add),
        TEASER_NoShuf(noise_range=d.noise_add),
...
def attempt(num_K, estims, num_extra=0, noise_range=1, delta=0.1, epsilon=0.1):
    ...
    sol_teaser2 = estims["teaser-noshuf"](q_pts, k_pts, noise_range=noise_range)
    ...
    sol_rts1 = estims["rts"](q_pts, k_pts)
    sol_rts2 = estims["rts-heuristic"](q_pts, k_pts)
    sol_clipperp = estims["clipperp"](q_pts, k_pts, delta, epsilon)
    sol_teaser1 = estims["teaser"](q_pts, k_pts, noise_range=noise_range)
    sol_goicp1 = estims["goicp"](q_pts, k_pts)
    sol_goicp2 = estims["goicp-scaled"](q_pts, k_pts, scale=zoom)
