...
class Base2DEstimator:
    __estim_name__ = None

    __default_params__ = set(["zoom", "theta", "dx", "dy"])
    ...
    def _call_impl(self, q_pts, k_pts, *args, **kwargs):
        raise NotImplementedError("subclass Base2DEstimator")
...
class GoICP(Base2DEstimator):
    ...
    def _call_impl(self, q_pts, k_pts, *args, **kwargs):
        ...
        e = self.__estim_name__
        sol[f"{e}_zoom"] = scale
        sol[f"{e}_theta"] = np.arctan2(rotmat[0, 1], rotmat[0, 0])
        sol[f"{e}_dx"] = transl[0]
        sol[f"{e}_dy"] = transl[1]
        return sol


class GoICPScaled(Base2DEstimator):
    ...
    def _call_impl(self, q_pts, k_pts, *args, **kwargs):
        ...
        sol = dict()
        e = self.__estim_name__
        sol[f"{e}_zoom"] = kwargs.get("scale", 1.0)
        sol[f"{e}_theta"] = np.arctan2(rotmat[0, 1], rotmat[0, 0])
        sol[f"{e}_dx"] = transl[0]
        sol[f"{e}_dy"] = transl[1]
        return sol
...
class TEASER(Base2DEstimator):
    ...
    def _call_impl(self, q_pts, k_pts, *args, **kwargs):
        ...
        sol[f"{e}_theta"] = np.arctan2(rotmat[1, 0], rotmat[1, 1])
...
class RTSHeuristic(Base2DEstimator):
