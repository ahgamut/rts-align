...
def viz_alignment(ipair, corr, title="alignment"):
    m = KabschMapping()
...
def save_outputs(output_dir, ipair, corr, prefix="align"):
    m = KabschMapping()
    ...
    q_path = os.path.join(output_dir, f"{prefix}-q-ALIGNED.png")
    k_path = os.path.join(output_dir, f"{prefix}-k-ALIGNED.png")
...
def runner(input_zip, delta, epsilon, lower_bound, rescale, visualize, output_dir=None):
    ...
    fig = viz_alignment(ipair, corr, bname)
    if visualize:
        plt.show()
    if output_dir is not None:
        ...
        fig.savefig(f"{bbase}-alignment.png", dpi=200)
        save_outputs(output_dir, ipair, corr, prefix=bbase)
